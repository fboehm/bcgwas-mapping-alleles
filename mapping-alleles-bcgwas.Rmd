...
genocount <- function(gv.actg) # gv.actg is a character vector with each entry a 2-letter genotype
  {
  foo<- strsplit(gv.actg, "")
  f2<- matrix(ncol = 2, data=unlist(foo), byrow=TRUE)
  apply(FUN = function(x)sum(x==f2[1]), X = f2, MARGIN=1)
  }


```

Now, try genoquery function.
