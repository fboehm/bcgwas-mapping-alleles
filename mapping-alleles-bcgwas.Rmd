...
split2<- function(x){xx<- t(split1(x)); return(xx)


genoquery <- function(rsid="", # rs.id is the snp id.
                    genofile = "geno.ord.RData", # the genotype file that is used in Nate's site. Use full path
                    textfile = "ForwardStrand_FinalReport.csv" # the file with Madison's data. Use full path
                    )
  {
  geno.ord<- getobj(genofile)
  smap<- read.csv(genofile, skip=9, colClasses = "character") 
# test.csv is the "head" (first ~50 lines ) of ForwardStrand_FinalReport.csv
  names(smap)[1] <- "snp.id"
  roi<- which(smap[,1] %in% rsid) # row of interest
  gv <- as.vector(smap[roi,c(-1,-11, -23, -32, -33)])
  subjid <- colnames(smap[,-1, -11, -23, -32, -33])
  # get genotype vector from text file for Madison samples  
  foo<- split2(gv)
  foo[foo == "-"]<- NA 
  levs<- levels(as.factor(foo))
  # output is the genotype count
  o1<- apply(X=foo, FUN = function(x){sum(x == levs[1])})
  o2 <- geno.ord[which(rownames(geno.ord) %in% rsid), colnames(geno.ord) %in% subjid]
  out<- list(o1, o2)
  names(out)<- c("text-file", "geno.ord")
  return(out) # names correspond to source of the data
  }

## get the genotypes from geno.ord
