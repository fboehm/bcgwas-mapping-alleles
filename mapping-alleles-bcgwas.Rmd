...
```{r "userfn-defined"}
userfn <- function(rs.id, g.ord = geno.ord, gm.actg = newdat)
  {
  # get subject ids
  sid<- intersect(names(gm.actg), colnames(g.ord))
  d1<- split1(gm.actg[gm.actg$snp.id ==rs.id,colnames(gm.actg)%in% sid])
  gpout<- genoparse(as.character(d1))
  go.vec<- as.vector(g.ord[rownames(g.ord) == rs.id, colnames(g.ord) %in% sid])
  out1 <- data.frame(gpout[[1]]$input_geno, go.vec, gpout[[1]]$genovec, gpout[[2]]$genovec)
  # define out2 as the mapping that outputs geno.ord's codings
  if (allequal(out1[,2], out1[,3])) out2 <- gpout[[1]]$allele_identities
  if (allequal(out1[,2], out1[,4])) out2 <- gpout[[2]]$allele_identities
  out <- list(out1, out2)
...
Todo: revise genoparse() so that out[[1]]$input_geno is only the ACTG genotypes 